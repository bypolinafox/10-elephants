...
final class SearchPageController: UIViewController {
    ...
    private lazy var ingregientCollectionView: UICollectionView = factory.makeIngredientCollectionView()
    ...
    private lazy var tapRecognizer = UITapGestureRecognizer(target: self, action: #selector(hideSuggestions))
...
extension SearchPageController: UISearchBarDelegate {
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(reloadSuggestions), object: nil)
...
extension SearchPageController {
    private func configureCollectionViewSources(){
        ...
        mealCollectionView.register(Constants.mealCellType, forCellWithReuseIdentifier: Constants.mealCellID)
        
        ingregientCollectionView.dataSource = ingridientDataSource
        ingregientCollectionView.delegate = ingridientDataSource
        ingregientCollectionView.register(Constants.ingredientCellType, forCellWithReuseIdentifier: Constants.ingredientCellID)
        
        resultCollectionView.dataSource = resultDataSource
        resultCollectionView.delegate = resultDataSource
        resultCollectionView.register(Constants.resultMealCellType, forCellWithReuseIdentifier: Constants.resultMealCellID)
    }
    ...
    private func configureMealSuggestions(){
        ...
        NSLayoutConstraint.activate([
            mealCollectionView.heightAnchor.constraint(equalToConstant: Constants.mealSuggestionHeight),
            mealCollectionView.widthAnchor.constraint(equalTo: mealSuggestionsStack.widthAnchor)
        ])
        
        NSLayoutConstraint.activate([
            mealSuggestionTitle.leadingAnchor.constraint(equalTo: mealSuggestionsStack.leadingAnchor, constant: Constants.sideInsetValue),
            mealSuggestionTitle.trailingAnchor.constraint(equalTo: mealSuggestionsStack.trailingAnchor, constant: -Constants.sideInsetValue)
        ])
    }
    ...
    private func configureIngredientSuggestions(){
        ...
        NSLayoutConstraint.activate([
            ingregientCollectionView.heightAnchor.constraint(equalToConstant: Constants.ingredientHeight),
            ingregientCollectionView.widthAnchor.constraint(equalTo: ingredientSuggestionsStack.widthAnchor)
        ])
        
        NSLayoutConstraint.activate([
            ingredientSuggestionTitle.leadingAnchor.constraint(equalTo: ingredientSuggestionsStack.leadingAnchor, constant: Constants.sideInsetValue),
            ingredientSuggestionTitle.trailingAnchor.constraint(equalTo: ingredientSuggestionsStack.trailingAnchor, constant: -Constants.sideInsetValue)
        ])
    }
